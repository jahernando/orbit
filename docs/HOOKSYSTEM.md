...
| 10 | `secretary_refresh` | `core/shell.py:_action_secretary_refresh` → `run_dash(silent=False)` | no | no | Regenera `📊panel/secretary/{agenda,projects,calendar,report-summary}.md` + `📊panel/ring/rings.md`. Tras F1–F5 (2026-05-19) el dashboard hot único es `agenda.md`; los outputs cold se agrupan por backend (secretary lee verdad, ring lee `ring.json`). `rings.md` fusiona today+next desde v0.39 (2026-05-21). Renombrado de `dash_render` en 2026-05-16. |
