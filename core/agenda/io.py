...
def _read_agenda(path: Path) -> dict:
    ...
    The ``## 📊 Cronogramas`` section is preserved as an opaque list of
    lines (``cronos`` key). The block is auto-generated by
    :func:`core.cronograma._refresh_agenda_cronos_section` — the parser
    just keeps it intact across round-trips.
    """
    if not path.exists():
