...
2. **Banner auto-generated** en cada `.md` derivado, refuerza el "view-only":
   ```html
   <!-- 🤖 Auto-generated by orbit · regenerated on `orbit dash` / `orbit save` · do not edit, changes will be lost on next refresh -->
   ```
   Comment HTML invisible al renderizar (Obsidian no muestra comments), visible al editar. Refuerza que el fichero es derivado sin requerir frontmatter ni renombrado.
