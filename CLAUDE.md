...
`views/secretary/` con viewers puros del workspace. Tras F1–F5 (2026-05-19), el dashboard hot único es `agenda.md` (counter + Hoy + Próximos 7 días); regenerado tras cada mutación (carril hot). Carril cold se reparte por backend: `📊panel/secretary/{projects,calendar,report-summary}.md` (lee verdad de proyectos) + `📊panel/ring/rings.md` (lee `ring.json`). Se refresca sólo en `commit_post` / `day_open` / `orbit dash`. Front-page del workspace: `workspace.md` estático (bootstrappeado por `orbit setup`). En cloud: `workspace.md → workspace.html` + `index.html` stub redirect (auto-open del browser preservado).
