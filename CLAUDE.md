...
**2. Secretary + dashboard cloud unificado** ([ADR-035](DECISIONS.md#adr-035--viewssecretary-como-viewers-del-workspace--dashboard-cloud-unificado), evolucionado por el dashboard refactor 2026-05-19)
`views/secretary/` con viewers puros del workspace. Tras F1–F5 (2026-05-19), el dashboard hot único es `agenda.md` (counter + Hoy + Próximos 7 días); regenerado tras cada mutación (carril hot). Carril cold se reparte por backend: `📊panel/secretary/{projects,calendar,cronos,hitos,logbook,report-summary}.md` (lee verdad de proyectos) + `📊panel/ring/rings.md` (lee `ring.json`). Se refresca sólo en `commit_post` / `day_open` / `orbit dash`. El contador `🏁 Hitos: ⚠️N vencidos · N próximos 30 días` del header de `agenda.md` enlaza `hitos.md` (tabla de hitos vencidos + próximos 30d: 🏁 · fecha (⚠️ si vencido) · proyecto · hito · barra del cronograma cuyo `deadline:` nombra el hito). Convención de columnas en las tablas de citas: col1 = tipo (📅/☐/🏁/💬), col2 = estado/alerta (🔔 alarma · ⏩ por-triar · ⚠️ vencida). Front-page del workspace: `workspace.md` estático (bootstrappeado por `orbit setup`). En cloud: `workspace.md → workspace.html` + `index.html` stub redirect (auto-open del browser preservado).

Config en `<workspace>/orbit.json`:
```json
"secretary": { "report_days": 14 }
```
