...
class TestCommands:
    ...
    def test_crono_add_does_not_touch_agenda(self, projects_dir):
        """Cronogramas are decoupled from agenda.md: `crono add` creates the
        crono file but writes nothing into agenda.md (the secretary/panel
        viewers read cronos/crono-*.md directly)."""
        from core.cronograma import run_crono_add
        proj = _make_project(projects_dir)
        base = _strip_emoji(proj.name)
        agenda_before = (proj / f"{base}-agenda.md").read_text()
        run_crono_add(project=proj.name, name="Mi Plan")
        agenda_after = (proj / f"{base}-agenda.md").read_text()
        assert agenda_after == agenda_before
        assert "## 📊 Cronogramas" not in agenda_after
        assert (proj / "cronos" / "crono-mi-plan.md").exists()
