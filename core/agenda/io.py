...
def _read_agenda(path: Path) -> dict:
    """Parse agenda.md → {header, tasks, milestones, events, reminders, cronos}.

    Indented lines (4+ spaces or tab) after an item are collected as
    ``notes`` (list of strings, without the leading whitespace).

    The ``## 📊 Cronogramas`` section (legacy embedded table) is preserved
    as an opaque list of lines (``cronos`` key) so old-format files parse
    cleanly. Cronogramas are now decoupled from agenda.md: nothing writes
    this table anymore, and the new-format writer drops it on migration.
    """
    if not path.exists():
    ...
    for line in lines:
        ...
        if section is None:
            ...
            elif line == _CRONO_HEADER:
                section = "cronos"; last_item = None
            else:
            ...
            # Opaque block: keep every line verbatim (incl. blanks) until
            # the next known header. Trailing blanks are trimmed on write.
            result["cronos"].append(line)
        elif not line.strip():
    ...
    # Trim trailing blank lines from preserved cronos blob
    while result["cronos"] and not result["cronos"][-1].strip():
        result["cronos"].pop()
