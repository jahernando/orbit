"""views/secretary — viewers del workspace dashboard.

Cada viewer es una función `generate(out_path)` pura: lee la verdad
(agenda.md, logbook.md, etc. de los proyectos del workspace), escribe un
.md en `📊panel/secretary/`, return. La orquestación (qué viewer correr
cuándo) vive en `orbit.run_dash` / `run_dash_hot` / `run_dash_cold`.

Cada `.md` generado lleva un banner markdown visible al inicio
(`views.autogen_banner(module)`) con el emoji del workspace, el módulo y
el timestamp de generación. Marca el fichero como derivado: los viewers
se regeneran en cada `orbit dash` / `orbit save`, así que cualquier
edición manual se pierde.

Config en `<workspace>/orbit.json`:

    "secretary": {
      "report_days": 14    // ventana de report-summary, [1, 365]
    }
"""

import json
from pathlib import Path


DEFAULT_REPORT_DAYS = 14
MIN_REPORT_DAYS, MAX_REPORT_DAYS = 1, 365


def _clamp_int(value, lo: int, hi: int, default: int) -> int:
    try:
        v = int(value)
    except (TypeError, ValueError):
        return default
    if lo <= v <= hi:
        return v
    return default


def _load_secretary_config(workspace_root: Path) -> dict:
    """Read <workspace>/orbit.json → 'secretary' section, return merged config.

    Defaults: report_days=14. Values are clamped to [MIN_*, MAX_*]; bad
    types or out-of-range fall back silently to defaults.
    """
    cfg = {"report_days": DEFAULT_REPORT_DAYS}
    path = workspace_root / "orbit.json"
    if not path.exists():
        return cfg
    try:
        data = json.loads(path.read_text())
    except (json.JSONDecodeError, OSError):
        return cfg
    user = data.get("secretary")
    if not isinstance(user, dict):
        return cfg
    if "report_days" in user:
        cfg["report_days"] = _clamp_int(
            user["report_days"], MIN_REPORT_DAYS, MAX_REPORT_DAYS,
            DEFAULT_REPORT_DAYS,
        )
    return cfg
