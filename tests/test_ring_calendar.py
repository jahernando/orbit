...
class TestScheduleNewFormatReminders:
    """schedule_new_format_reminders is now a no-op (declarative model).

    Tasks/milestones/reminders flow continuously via gsync.sync_item hooks
    into Reminders.app as completable items. Events use Calendar.app's
    native display alarms. No daily startup-time scheduling needed.
    """
    ...
    def test_returns_empty_no_op(self, tmp_path, monkeypatch):
        """Function is now a no-op regardless of inputs."""
        self._patch_projects(tmp_path, monkeypatch)
        assert schedule_new_format_reminders(date(2026, 4, 4)) == []
    ...
    # The four tests below used to assert the old behaviour
    # (startup-time scheduling of ring alarms in Reminders.app). After the
    # declarative-sync refactor (Phase 2A) the function always returns [],
    # so they are kept as light-weight smoke tests of the no-op path.

    def test_schedules_ringing_task_now_no_op(self, tmp_path, monkeypatch):
    ...
    def test_does_not_clear_ring_anymore(self, tmp_path, monkeypatch):
        """No-op must NOT mutate agenda.md (the old behaviour cleared the ring)."""
        type_dir = self._patch_projects(tmp_path, monkeypatch)
        pd = _make_project(type_dir, "alpha")
        (pd / "alpha-agenda.md").write_text(
            "## ✅ Tareas\n"
        ...
        from core.agenda_cmds import _read_agenda
        data = _read_agenda(pd / "alpha-agenda.md")
        # ring stays as the user set it — gsync handles it via Reminders.app
    ...
    def test_no_op_returns_empty_regardless_of_failure_mock(self, tmp_path, monkeypatch):
        """No call into _schedule_reminder happens anymore."""
        type_dir = self._patch_projects(tmp_path, monkeypatch)
        pd = _make_project(type_dir, "alpha")
        (pd / "alpha-agenda.md").write_text(
            "## ✅ Tareas\n"
        ...
        with patch.object(ring_mod, "_schedule_reminder",
                          return_value=False) as mock:
            assert schedule_new_format_reminders(date(2026, 4, 4)) == []
        mock.assert_not_called()

    def test_non_new_format_project_ignored(self, tmp_path, monkeypatch):
        """Old-format projects (no {name}-agenda.md) — no calls regardless."""
        type_dir = self._patch_projects(tmp_path, monkeypatch)
    ...
    def test_multiple_projects_no_op(self, tmp_path, monkeypatch):
        type_dir = self._patch_projects(tmp_path, monkeypatch)
